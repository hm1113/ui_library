...
/**
 * Presents load'n'display image task. Used to load image from Internet or file system, decode it to {@link Bitmap}, and display it in
...
final class LoadAndDisplayImageTask implements Runnable {
    ...
    public void run() {
        ...
        log(LoadAndDisplayImageTask.LOG_START_DISPLAY_IMAGE_TASK);
        if (loadFromUriLock.isLocked()) {
            log(LoadAndDisplayImageTask.LOG_WAITING_FOR_IMAGE_LOADED);
        }
        ...
        try {
            ...
            if (bmp == null) {
                ...
                if (options.shouldPreProcess()) {
                    log(LoadAndDisplayImageTask.LOG_PREPROCESS_IMAGE);
                    bmp = options.getPreProcessor().process(bmp);
                    if (bmp == null) {
                        L.e(LoadAndDisplayImageTask.ERROR_PRE_PROCESSOR_NULL);
                    }
                }

                if (bmp != null && options.isCacheInMemory()) {
                    log(LoadAndDisplayImageTask.LOG_CACHE_IMAGE_IN_MEMORY);
                    configuration.memoryCache.put(memoryCacheKey, bmp);
                }
            } else {
                loadedFrom = LoadedFrom.MEMORY_CACHE;
                log(LoadAndDisplayImageTask.LOG_GET_IMAGE_FROM_MEMORY_CACHE_AFTER_WAITING);
            }

            if (bmp != null && options.shouldPostProcess()) {
                log(LoadAndDisplayImageTask.LOG_POSTPROCESS_IMAGE);
                bmp = options.getPostProcessor().process(bmp);
                if (bmp == null) {
                    L.e(LoadAndDisplayImageTask.ERROR_POST_PROCESSOR_NULL, memoryCacheKey);
                }
            }
        } finally {
    ...
    private boolean waitIfPaused() {
        ...
        synchronized (pause) {
            if (pause.get()) {
                log(LoadAndDisplayImageTask.LOG_WAITING_FOR_RESUME);
                try {
                    pause.wait();
                } catch (final InterruptedException e) {
                    L.e(LoadAndDisplayImageTask.LOG_TASK_INTERRUPTED, memoryCacheKey);
                    return true;
                }
                log(LoadAndDisplayImageTask.LOG_RESUME_AFTER_PAUSE);
            }
        }
    ...
    private boolean delayIfNeed() {
        if (options.shouldDelayBeforeLoading()) {
            log(LoadAndDisplayImageTask.LOG_DELAY_BEFORE_LOADING, options.getDelayBeforeLoading(), memoryCacheKey);
            try {
    ...
    /**
     * Check whether target ImageView wasn't collected by GC and the image URI of this task matches to image URI which is actual for current ImageView
     * at this moment and fire {@link ImageLoadingListener#onLoadingCancelled(String, android.view.View)} event if it doesn't.
     */
    ...
    private ImageView checkImageViewRef() {
        ...
        if (imageView == null) {
            ...
            log(LoadAndDisplayImageTask.LOG_TASK_CANCELLED_IMAGEVIEW_LOST);
            fireCancelEvent();
        }
        return imageView;
    }
    ...
    private boolean checkImageViewReused(final ImageView imageView) {
        ...
        if (imageViewWasReused) {
            log(LoadAndDisplayImageTask.LOG_TASK_CANCELLED_IMAGEVIEW_REUSED);
            fireCancelEvent();
        }
    ...
    private boolean checkTaskIsInterrupted() {
        ...
        if (interrupted) {
            log(LoadAndDisplayImageTask.LOG_TASK_INTERRUPTED);
        }
        return interrupted;
    }
    ...
    private Bitmap tryLoadBitmap() {
        ...
        try {
            if (imageFile.exists()) {
                log(LoadAndDisplayImageTask.LOG_LOAD_IMAGE_FROM_DISC_CACHE);
            ...
            if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                log(LoadAndDisplayImageTask.LOG_LOAD_IMAGE_FROM_NETWORK);
    ...
    private Bitmap decodeImage(final String imageUri) throws IOException {
        ...
        final ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey, imageUri, targetSize, viewScaleType, getDownloader(), options);
    ...
    private String tryCacheImageOnDisc(final File targetFile) {
        log(LoadAndDisplayImageTask.LOG_CACHE_IMAGE_ON_DISC);
    ...
    private boolean downloadSizedImage(final File targetFile, final int maxWidth, final int maxHeight) throws IOException {
        // Download, decode, compress and save image
        final ImageSize targetImageSize = new ImageSize(maxWidth, maxHeight);
        final DisplayImageOptions specialOptions = new DisplayImageOptions.Builder().cloneFrom(options).imageScaleType(ImageScaleType.IN_SAMPLE_INT)
                .build();
        final ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey, uri, targetImageSize, ViewScaleType.FIT_INSIDE, getDownloader(),
                specialOptions);
        ...
        if (configuration.processorForDiscCache != null) {
            log(LoadAndDisplayImageTask.LOG_PROCESS_IMAGE_BEFORE_CACHE_ON_DISC);
            bmp = configuration.processorForDiscCache.process(bmp);
            if (bmp == null) {
                L.e(LoadAndDisplayImageTask.ERROR_PROCESSOR_FOR_DISC_CACHE_NULL, memoryCacheKey);
                return false;
            }
        }

        final OutputStream os = new BufferedOutputStream(new FileOutputStream(targetFile), LoadAndDisplayImageTask.BUFFER_SIZE);
